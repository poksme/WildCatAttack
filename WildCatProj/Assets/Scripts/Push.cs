...
public class Push : MonoBehaviour {
	...
	void OnCollisionInChildren(Collision collision) {
		...
		if ((me.CompareTag("Player1") && other.CompareTag("Player2")) || 
		    (me.CompareTag("Player2") && other.CompareTag("Player1"))) { // Need to check if in dash state
			Debug.Log("Collision between players");
			if (collision.collider.rigidbody && me.GetComponent<Dash>().IsDashing && !other.GetComponent<Dash>().IsDashing) // The feet don't have rigidbody this can be a problem 
			{
